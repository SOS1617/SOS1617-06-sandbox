...
app.get(BASE_API_PATH + "/contacts", function(request, response) {
    ...
    db.find({}).toArray(function(err, contacts) {
        if (err) {
        ...
        db.find({"name" : name}, function(err, contacts) {
            if (err) {
                ...
                /*var filteredContacts = contacts.filter((contact) => {
                    return (contact.name.localeCompare(name, "en", {
                        'sensitivity': 'base'
                    }) === 0);
                });*/
                if (contacts.length > 0) {
                    var contact = contacts[0]; //since we expect to have exactly ONE contact with this name
            ...
            db.find({}, function(err, contacts) {
                if (err) {
                    ...
                    var contactsBeforeInsertion = contacts.filter((contact) => {
                        return (contact.name.localeCompare(newContact.name, "en", {
                            'sensitivity': 'base'
                        }) === 0);
                    });
...
app.put(BASE_API_PATH + "/contacts/:name", function(request, response) {
    ...
    if (!updatedContact) {
        ...
        console.log("INFO: New PUT request to /contacts/" + nameParam + " with data " + JSON.stringify(updatedContact, 2, null));
        if (!updatedContact.name || !updatedContact.phone || !updatedContact.email) {
            ...
            db.find({name:nameParam}, function(err, contacts) {
                if (err) {
                    ...
                    /* contactsBeforeInsertion = contacts.filter((contact) => {
                        return (contact.name.localeCompare(nameParam, "en", {
                            'sensitivity': 'base'
                        }) === 0);
                    });*/
                    if (contacts.length > 0) {
                        ...
                        console.log("INFO: Modifying contact with name " + nameParam + " with data " + JSON.stringify(updatedContact, 2, null));
                        response.send(updatedContact); // return the updated contact
                    }
                    else {
                        console.log("WARNING: There are not any contact with name " + nameParam);
                        response.sendStatus(404); // not found
                    }
                }
            });
